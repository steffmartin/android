...
public class MeiosDeTransporteDAO {
    ...
    public MeioDeTransporte buscaMeioDeTransporte(int id){
        SQLiteDatabase banco = db.getInstance(mContext).getReadableDatabase();
        Cursor cursor;
        String particular = "SELECT * FROM Particular WHERE _id = '" + id + "'";
        String alugado = "SELECT * FROM Alugado WHERE _id = '" + id + "'";
        String compartilhado = "SELECT * FROM Compartilhado WHERE _id = '" + id + "'";
        String publico = "SELECT * FROM Publico WHERE _id = '" + id + "'";
